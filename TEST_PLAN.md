...
- [ ] `/admin/sys-deps` 出現「OxOffice / LibreOffice 執行時依賴 X11 lib」項目，全綠
...
- [ ] 「02-2345-6789 #123」→ 偵測到 `landline` 含分機
- [ ] 「新北市板橋區中山路一段 100 號 5 樓之 1」→ 偵測到 `addr` 含「樓之」
...
### 6.13 v1.4.43 ~ v1.4.53 一連串改動

#### 6.13.1 install.ps1 BOM + WinSW 移轉 (v1.4.43~v1.4.48)

- [ ] install.ps1 開頭 3 byte = `EF BB BF`（UTF-8 BOM）
- [ ] Win11 PowerShell `[Parser]::ParseFile(install.ps1, ...)` 回 SYNTAX_OK
- [ ] Fresh Win11 install → bin/jtdt-svc.exe 出現、bin/jtdt-svc.xml 出現、`sc qc jt-doc-tools` BINARY_PATH 指向 jtdt-svc.exe
- [ ] 既有 NSSM 安裝（v1.4.43-）跑 `jtdt update` → 兩次後完成移轉，`sc qc` BINARY_PATH 換成 jtdt-svc.exe，env 變數 (JTDT_HOST/PORT/DATA_DIR) 保留
- [ ] WinSW XML 含 `<onfailure>` x3（10s/20s/60s 漸進重試）
- [ ] `jtdt uninstall --purge` 透過 `winsw uninstall` 把服務 + XML + log 一起清掉

#### 6.13.2 LDAP 使用者操作不再匿名（v1.4.50~v1.4.51）

- [ ] LDAP 帳號登入後跑 pdf-stamp / pdf-watermark → admin/history/{stamp,watermark} 看到 `jason@ldap`（不是 `(匿名)`）
- [ ] local 帳號顯示 `jason@local`
- [ ] 同一個帳號名 jason 同時存在 local 跟 ldap → 兩筆記錄分得清楚

#### 6.13.3 新工具自動加入適合的預設角色（v1.4.51）

- [ ] 模擬升級：用 v1.4.40 安裝、登入 `default-user` 角色 → 之後升 v1.4.51+ → 該角色 role_perms 自動補上 v1.4.x 後加的所有工具（包括 text-deident）
- [ ] admin 手動加入該 role 的 grants 不會被洗掉（only ADD never REMOVE）
- [ ] admin 手動移除該 role 的 grants → 升級後會被「補回來」（已知 trade-off，CHANGELOG 註明）

#### 6.13.4 文字去識別化（text-deident，v1.4.49~v1.4.53）

- [ ] 工具列表（首頁 + 搜尋）能找到「文字去識別化」
- [ ] 貼一段含 1) 王經理 2) 0912-345-678 3) test@example.com 4) A123456789 5) 統編 12345670 的文字
- [ ] 偵測 → 看到 5 筆找出，type 對應正確
- [ ] 切「遮罩」→ 套用 → 5 筆都變 mask 字串（W*M、09*****-678 等）
- [ ] 切「編修」→ 套用 → 5 筆都變黑塊 █████
- [ ] 切「替換假資料」→ 套用 → 變實際的 random 假姓名 / 假號碼，且結構保留
- [ ] 上傳 .docx / .odt → 自動透過 OxOffice 解析成文字
- [ ] 「自訂 regex」展開後輸入「員工編號 \| EMP-\d{6}」→ 在文字中加 `EMP-123456` → 偵測勾選自訂後找到
- [ ] 切「IT 資料」→ 全選 → 偵測「server 192.168.1.1, user ACME\jcheng, ghp_abc..." → 至少抓到 3 個 IT findings
- [ ] LLM 補偵測勾選後（須先啟 LLM），偵測時印「[LLM] xxx」類別

#### 6.13.5 頁面轉向 UX 大改（v1.4.53）

- [ ] 上傳 PDF → 自動顯示縮圖（不需按鈕）
- [ ] 套用範圍下拉選「偶數頁」→ count 顯示「N 頁將被套用」
- [ ] 點上方 90° 按鈕 → 偶數頁全部變 90° 旋轉（縮圖即時更新）
- [ ] 套用範圍切「自填頁碼」→ 輸入 `1,3-5` → 顯示 4 頁
- [ ] 點某張縮圖下方的單頁按鈕 → 該頁覆寫
- [ ] 「下載 PDF」→ 出 PDF，rotation 正確
- [ ] 「下載 ZIP（每頁 PNG）」→ ZIP 內 page_001.png ... 每張對應 baked rotation
- [ ] 「處理新檔案」→ UI reset，可上傳下一份

#### 6.13.6 macOS install.sh 直接 root 執行（v1.4.53）

- [ ] 在 macOS 切 root（`sudo su -`）然後跑 `bash install.sh` → 偵測 `/dev/console` user → warn 後繼續用該 user 當 .app 擁有者
- [ ] 桌面真的沒人登入 → die「找不到非 root 桌面使用者」
- [ ] sudo 路徑（`sudo bash install.sh`）行為不變
