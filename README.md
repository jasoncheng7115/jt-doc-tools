...
```powershell
$u='https://raw.githubusercontent.com/jasoncheng7115/jt-doc-tools/main/install.ps1?cb='+[int][double]::Parse((Get-Date -UFormat %s)); $f="$env:TEMP\jtdt-install.ps1"; [Net.ServicePointManager]::SecurityProtocol='Tls12'; $d=(New-Object Net.WebClient).DownloadData($u); $b=New-Object byte[] ($d.Length+3); $b[0]=0xEF;$b[1]=0xBB;$b[2]=0xBF; [Array]::Copy($d,0,$b,3,$d.Length); [IO.File]::WriteAllBytes($f,$b); powershell -NoProfile -ExecutionPolicy Bypass -File $f; Read-Host '安裝結束，按 Enter 關閉'
```
