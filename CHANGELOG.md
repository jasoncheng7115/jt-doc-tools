...
- **Sprint 4**：L3 LLM 文字層 — `submission-check` 列入 `KNOWN_LLM_TOOLS`：(a) **fuzzy 變體合併**（光寶 ↔ 光寶科技 ↔ LITE-ON 自動聚成一群避免誤標 mismatch），(b) **修改範本痕跡推論**（讀前 500 字判斷哪些檔疑似從別案範本沿用未改）。沒設定 LLM 自動 fallback 不影響整體運作。
